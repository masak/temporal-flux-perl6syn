...
This document outlines built-in Perl 6 support for times, dates and durations.
...
Identical to a call to C<localtime> with a C<$timezone> argument value of
C<'GMT'>.

=item localtime

 our Temporal::DateTime multi localtime ( Num $epoch, Str $timezone? )
 our Temporal::DateTime multi localtime ( Rat $epoch = time(), Str $timezone? )
...
The intent behind these classes is to provide a minimal,
